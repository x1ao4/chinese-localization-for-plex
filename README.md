...
6. 使用命令 `python3 plex-localization-zh.py --new` 可运行 `处理新增项目` 任务，脚本将启动一个 Flask 服务器来监听 Plex 服务器的事件，当 Plex 服务器上有新增项目时，脚本将自动对新增项目进行处理，并在控制台显示处理结果，处理完毕后会继续监听 Plex 服务器的事件，并在每次有新增项目时对其进行处理，然后继续监听。
